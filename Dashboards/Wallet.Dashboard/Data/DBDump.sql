...
(
 ...
 CONSTRAINT [FK_Transactions_TransactionStatusID] FOREIGN KEY ([TransactionStatusID])  REFERENCES [dbo].[TransactionStatus]([ID]),
...
(
 [ID]             int IDENTITY (1, 1) NOT NULL ,
 [UserID]         int NOT NULL ,
 [Balance]        decimal(18,0) NOT NULL CONSTRAINT [DF_SavedPaymentMethods_Balance] DEFAULT 0 ,
...
(
 ...
 CONSTRAINT [FK_SavedCards_CardTypeID] FOREIGN KEY ([CardTypeID])  REFERENCES [dbo].[CardTypes]([ID]),
 CONSTRAINT [FK_SavedCards_UserID] FOREIGN KEY ([UserID])  REFERENCES [dbo].[Users]([ID])
);
GO


CREATE NONCLUSTERED INDEX [fkIdx_SavedCards_CardTypeID] ON [dbo].[SavedCards] 
 (
  [CardTypeID] ASC
 )
